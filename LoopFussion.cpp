...
void fuseLoops(Loop *Lj, Loop *Lk, LoopInfo &LI, ScalarEvolution &SE) {
    ...
    //ricavo gli ultimi blocchi del body di ciascun loop che contengono l'istruzione salto per il latch
    ...
    Term2->eraseFromParent();
        ...
        // Rimuovere il branch dal header del loop 2 all'uscita del loop 2
    }
    ...
    // Collegare l'header del loop 1 all'uscita del loop 2
    // q: cosa fa il metodo Create()?
    // a: crea un'istruzione di branch
    BranchInst::Create(Header1, Exit2);
    Header1->getTerminator()->setSuccessor(0, Exit2);
    
    //collegare il body del loop 1 al body del loop 2
    BranchInst::Create(LastB1, FirstB2);
    LastB1->getTerminator()->setSuccessor(0, FirstB2);
    
    // Collegare il body del loop 2 al latch del loop 1
    BranchInst::Create(LastB2, Latch1);
    LastB2->getTerminator()->setSuccessor(0, Latch1);
    
    //collegare l'header del loop 2 al latch del loop 2
    BranchInst::Create(Header2, Latch2);
    Header2->getTerminator()->setSuccessor(0, Latch2);
}
